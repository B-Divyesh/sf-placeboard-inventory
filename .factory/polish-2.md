...
- Deployment: Azure Static Web Apps deployment `65ed4721-5b2d-458c-8f07-fef09c475459` succeeded. Cold live `verify-url.sh` passed on `/` and `/demo`; evidence is in `.factory/evidence-polish-2/live-root/` and `.factory/evidence-polish-2/live-demo/`. Live 1440×900 first-screen ends were 681, 725, 778, 809, and 840px respectively; the seven-route live Axe audit found zero serious or critical violations; direct `?demo=1` reset then Start for real reached `/inventory`; `/definitely-missing-review-route` returned HTTP 404 with one h1 and legal links.
